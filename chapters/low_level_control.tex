...
\label{chap:low_level_control_design}
...
for control design. Controls are designed referencing Nise\cite{nise_control} to source general forms
for systems and terms.
...
Taking inspiration from Ioannou et al,\cite{ioannou_aicc}, the dynamic model can be simplified based on the domain the error signal exists in.
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[auto]
        % Nodes
        \node[sum] (sum) {};
        \node[left=1.5cm of sum] (r) {$V^*(s)$};
        \node[block, right=1.2cm of sum] (cblock) {$C(s)$};
        \node[block, right=1.2cm of cblock] (gblock) {$G(s)$};
        \node[right=1.2cm of gblock] (v) {$V(s)$};

        % Sum node labels
        \node at ($(sum) + (-0.15,0.15)$) {\small $+$};
        \node at ($(sum) + (-0.15,-0.15)$) {\small $-$};

        % Forward path
        \draw[->] (r) -- (sum);
        \draw[->] (sum) -- node[above] {$E(s)$} (cblock);
        \draw[->] (cblock) -- (gblock);
        \draw[->] (gblock) -- node[above] {$V(s)$} (v);

        % Feedback path (unit feedback, no block)
        \draw[->] ([xshift=0.25cm]gblock.east) |- ([yshift=-1.2cm]gblock.east) -| (sum.south);
    \end{tikzpicture}
    \caption{Unity feedback speed control loop with controller $C(s)$ and plant $G(s)$.}
    \label{fig:unity_feedback_speed}
\end{figure}
